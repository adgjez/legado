...
object AiVideoService {
    ...
    // ============================================================
    // P4 薄壳化：新 generate 入口（老 submit/poll/downloadToLocal 共存）
    // ============================================================

    /**
     * P4 薄壳生成入口：resolveProvider → VideoBackendRegistry.byConfig →
     * resolvePayloadLimits → buildSpecs → MediaGenerator.runWithReferenceCompression { backend.generate }。
     *
     * 与老 [submit] + [poll] + [downloadToLocal] 三段式 API 共存：
     * - 老调用点（[AiVideoTaskPoller]）仍用三段式 API，不破
     * - 新调用点（P6 改造 NovelVideoGenerator）用本方法，一气呵成
     *
