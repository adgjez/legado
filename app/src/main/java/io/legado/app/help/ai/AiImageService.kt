...
object AiImageService {
    ...
    // ============================================================
    // P4 薄壳化：新 generate 入口（老 generate/generateAndStore 共存）
    // ============================================================

    /**
     * P4 薄壳生成入口：resolveProvider → ImageBackendRegistry.byConfig →
     * resolvePayloadLimits → MediaGenerator.runWithReferenceCompression { backend.generate }。
     *
     * 与老 [generate]（String prompt → String source）共存：
     * - 老调用点（NovelVideoGenerator Stage 5）仍用老 [generateAndStore]，不破
     * - 新调用点（P6 改造）用本方法，所有参考图 [RefRole.ARRAY] 角色走压缩梯子
     *
    ...
    /**
     * P4 薄壳 [generateAndStore]：调 [generate] 后存 [AiImageGalleryManager]。
     *
     * 与老 [generateAndStore]（String prompt → AiGeneratedImage）共存。
